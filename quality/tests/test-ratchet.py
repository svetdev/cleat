...
code, out = report(one, strict=True)
check("a single-measurement gate's improvement is a note under --strict, not a failure", code == 0 and "improved" in out and "looser" not in out, out)
one_stale = ratchet.judge([], [{"file": ".", "text": "share", "percent": 1.10}], ["percent"])
code, out = report(one_stale, strict=True)
check("but its stale entry still is", code == 1, out)
