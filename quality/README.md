...
| `bin/gate.py` | not a check: runs every gate the config names; `--strict` for CI, `--hook` and `--guard` for an agent's hooks, `--changed` scopes the heavy gates to the files changed against the base (the hook's default), `CLEAT_HOOKS=off` in the environment stands both hook modes down for a session that only reads, `--stats` for what those two did (each firing is a line in `quality/.events.jsonl`, gitignored; a refused call records its target, an allowed one only the tool; `"events": false` in the config turns it off) | — |
